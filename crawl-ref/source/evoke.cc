...
static void _fiery_explosion()
{
    ...
    beam.colour        = RED;
    beam.aux_source    = "an exploding Shard of Destruction";
    beam.ex_size       = 2;
...
static bool _shard_of_destruction(item_def &item)
{
    ...
    count_action(CACT_EVOKE, EVOC_SHARD);
    ...
    else if (x_chance_in_y(3, 39))
    {
        if (one_chance_in(5))
        {
            mprf("The shard cracks and shatters in your %s with a final"
                 " outpouring of destructive magic!",
