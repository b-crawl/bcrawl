...
spret cast_sticks_to_snakes(int pow, god_type god, bool fail)
{
    ...
    if (count)
    {
        ...
        if (sticks_left)
            mprf("You now have arrows for %d snake%s.", sticks_left,
                                             sticks_left > 1 ? "s" : "");
        else
 ...
 * @returns spret::abort if no viable corpse was at the player's location,
...
bool battlesphere_can_mirror(spell_type spell)
{
    ...
    case SPELL_FREEZING_CLOUD:
        return false;
    default:
        return spell_typematch(spell, SPTYP_CONJURATION) && spell_to_zap(spell) != NUM_ZAPS;
    }
}
