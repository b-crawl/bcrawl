...
random_var player::attack_delay(const item_def *projectile, bool rescale) const
{
    ...
    if (projectile && is_launched(this, weap, *projectile) == launch_retval::THROWN)
    {
        ...
        attk_delay = rv::max(attk_delay, random_var(min_proj_delay));
    }
    else if (!projectile && !weap)
    {
        int sk = form_uses_xl() ? experience_level * 10 : skill(SK_UNARMED_COMBAT, 10);
        attk_delay = div_rand_round(random_var(2700), sk + 270);
                            ...
                            string *reason, bool temp) const
{
    ...
    else if (is_lifeless_undead(temp) && you.species != SP_GHOUL)
