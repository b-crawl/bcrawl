...
int slaying_bonus(bool ranged)
{
    ...
    if (ranged && you.wearing_ego(EQ_GLOVES, SPARM_ARCHERY))
        ret += 4;

    ret += augmentation_amount();
...
bool player_regenerates_mp()
{
    ...
    if (you.spirit_shield() && you.species == SP_DEEP_DWARF)
        return false;
    // Pakellas blocked MP regeneration.
...
int player::item_adjusted_shield_penalty (int base_shield_penalty, int scale) const
{
...
int player::adjusted_shield_penalty(int scale) const
{
    ...
    int base_shield_penalty = -property(*shield_l, PARM_EVASION);
    return item_adjusted_shield_penalty(base_shield_penalty, scale);
}
...
int player::skill(skill_type sk_input, int scale, bool real, bool drained, bool temp) const
{
    ...
    {
        int skill_floor = 0;
        int invo = you.skill(SK_INVOCATIONS, scale, true, false, false);
...
{
    return (species == SP_AVARIEL) && airborne();
}
...
bool player::polymorph(int pow)
{
    ...
    if (f != transformation::none && transform(pow, f, true, false))
    {
