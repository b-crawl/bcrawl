...
void torment_player(actor *attacker, torment_source_type taux)
{
    ...
    if (!player_res_torment())
    {
        ...
        if (you.form == transformation::statue || you.has_mutation(MUT_HALF_TORMENT_RESISTANCE)
...
spret cast_random_effects(int pow, bolt& beam, bool fail)
{
    ...
    zap_type zap = random_choose(ZAP_INNER_FLAME,
                                 ...
                                 ZAP_VENOM_BOLT,
                                 ZAP_MALMUTATE);
    if (zap == ZAP_BOLT_OF_DRAINING && god_hates_action(DID_EVIL, you.religion))
        goto selection_loop;
    if (zap == ZAP_MALMUTATE && god_hates_action(DID_CHAOS, you.religion))
        goto selection_loop;
