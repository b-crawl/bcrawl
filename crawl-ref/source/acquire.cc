...
static int _acquirement_misc_subtype(bool /*divine*/, int & /*quantity*/)
{
    ...
    const vector<pair<int, int> > choices =
    {
        ...
        {MISC_BOX_OF_BEASTS,  (NO_LOVE ?     0 :  8)},
        {MISC_PHANTOM_MIRROR, (NO_LOVE ?     0 :  6)},
        // The player never needs more than one.
        {MISC_SACK_OF_SPIDERS,
            ((NO_LOVE || you.seen_misc[MISC_SACK_OF_SPIDERS]) ? 0 : 9)},
        {MISC_PHIAL_OF_FLOODS,
            ((NO_LOVE || you.seen_misc[MISC_PHIAL_OF_FLOODS]) ? 0 : 17)},
        {MISC_LIGHTNING_ROD,
    ...
    if(class_wanted == OBJ_RANDOM)
    {
        ...
        while (true)
        {
            if (crawl_state.seen_hups)
            {
                ...
                mprf(MSGCH_WARN, "Cancelling acquirement.");
                return false;
            }
            ...
            if (viewing)
                mprf(MSGCH_PROMPT, "\nView which item? (Press ! for selection.)");
            else
            ...
            for (unsigned int i = 0; i < stock.size(); ++i)
            {
                ...
                if (item_name[0] != '!')
                    item_strings.push_back(make_stringf("%c: %s", selection_key, item_name.c_str()));
                else
                    failed = true;
            ...
            {
                key = tolower(key) - 'a';
                if (key >= static_cast<int>(stock.size()) || key < 0)
                {
                    mpr("No such item.");
                    continue;
                }
                ...
                else
                {
                    ...
                    if (yesno(NULL, true, 'n', false, false, true))
                    {
                        for (unsigned int i = 0; i < selected.size(); i++)
                        {
                            ...
                            if (selected[i])
                            {
                                // ID it before giving it to the player.
                                set_ident_type(item, true);
                                set_ident_flags(item, ISFLAG_IDENT_MASK);
