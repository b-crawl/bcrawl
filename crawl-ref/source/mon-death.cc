...
static int _calc_player_experience(const monster* mons)
{
    ...
    if (!experience || you.species == SP_FELID)
        return 0;
...
void record_monster_defeat(const monster* mons, killer_type killer)
{
    ...
    if (mons->is_named() && mons->friendly()
        && !mons_is_hepliaklqana_ancestor(mons->type) && mons->type != MONS_PLAYER_ILLUSION)
    {
                                ...
                                int killer_index, bool maybe_good_kill)
{
    ...
    // Jiyva hates you killing slimes, but eyeballs mutation can confuse without you meaning it.
    // Also, don't anger Jiyva when your clouds accidentally kill slimes.
    if (mons_is_slime(mons) && killer != KILL_YOU_CONF && mons.attitude != ATT_FRIENDLY)
