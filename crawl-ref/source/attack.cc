...
int attack::calc_to_hit(bool random)
{
    ...
    if (attacker->is_player())
    {
        ...
        {
            // small base accuracy for unarmed and throwing
            mhit += 2;
...
bool attack::distortion_affects_defender()
{
    ...
    const disto_effect choice = random_choose_weighted(36, SMALL_DMG,
                                                       23, BIG_DMG,
                                                       17, BLINK,
                                                       6, TELE_INSTANT,
                                                       12, TELE_DELAYED,
...
void attack::pain_affects_defender()
{
    ...
    int skill = user->skill_rdiv(SK_NECROMANCY) + 1;

    if (!one_chance_in(skill))
    {
        special_damage += resist_adjust_damage(defender, BEAM_NEG, random2(skill));
...
int attack::player_stat_modify_damage(int damage)
{
    ...
    int strength_factor = 26 + you.strength();
