...
void ash_check_bondage(bool msg)
{
    ...
    for (int j = EQ_FIRST_EQUIP; j < NUM_EQUIP; j++)
    {
        ...
        else if (you.species != SP_OCTOPODE && i >= EQ_RING_ONE && i <= EQ_RING_EIGHT)
            continue;
        // The macabre finger necklace's extra slot does count if equipped.
        else if (!player_equip_unrand(UNRAND_FINGER_AMULET) && i == EQ_RING_AMULET)
            continue;
        
        // transformed away slots are still considered to be possibly bound
        // How are empty slots treated? Half value?
        if (you_can_wear(i))
            slot_total++;
        else continue;
        ...
        case EQ_WEAPON:
            item_val++;
            if (_two_handed() || you.get_mutation_level(MUT_MISSING_HAND))
                item_val++;
            break;
        ...
        // ring slots are worth more, but only for non-octopodes
        case EQ_LEFT_RING:
        case EQ_RIGHT_RING:
            item_val++;
            break;
        ...
        item_val *= 2;
        slot_total += item_val - 2;
        if (item.cursed())
            curse_total += item_val;
    }

    you.bondage_level = (curse_total * 100) / slot_total;
    /**
    you.skill_boost.clear();
    vector<skill_type> boosted_skills
    {
        SK_FIGHTING,
    };
    for (auto sk : boosted_skills)
    {
...
{
    return have_passive(passive_t::bondage_skill_boost) && you.bondage_level > 0;
}
...
unsigned int ash_skill_point_boost(skill_type sk, int scaled_skill)
{
    ...
    unsigned int skill_points = you.bondage_level * 0.05 * max(scaled_skill, 0)
                                           ...
                                           bool wall_jump)
{
    ...
    if (you.attribute[ATTR_SERPENTS_LASH] == 0)
    {
        int mv = 10;  // attacks per move are based on 10 speed when Lash is active
        you.increase_duration(DUR_EXHAUSTED, div_rand_round(mv*7, 10));
        drain_player(div_rand_round(mv*400, you.piety), false, true);
